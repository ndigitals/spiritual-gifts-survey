...
function spiritual_gifts_settings_page() { /*I have to exit PHP to write all the HTML*/ ?> 
    <div class="wrap">
        ...
        <h3>Options</h3>
        If you do not specify an email address, then the site's admin email <span style="font-style: italic"><?php print get_bloginfo( 'admin_email', 'Display'); ?></span> will be used.  To specify an email address, add the 'email' tag to your shortcode... <code>[spiritual_gifts email="your@email.com"]</code>
        <h3>Materials</h3>
...
//    echo "current: ".curPageURL();
    ...
    */
    if (curPageURL() == $_SERVER['HTTP_REFERER']."?submitted" ) {
        return "Thank you, your survey has been submitted.  Take <a href='".$_SERVER['HTTP_REFERER']."'>another one</a>?";
    } else {
        extract( shortcode_atts( array(
            'email' => get_bloginfo( 'admin_email', 'Display'),
        ), $atts ) );
        spiritual_gifts_write_form( $email );
    }
}
add_shortcode( 'spiritual_gifts', 'spiritual_gifts_shortcode_func' );

//write the form
function spiritual_gifts_write_form( $email ) {
    $return_var = "";
    ...
    <script type='text/javascript'>
        ...
        function previewResults(typeCount) {
            ...
            var keyName;
            document.getElementById('thisPreviewResults').style.display = 'block';
...
//                document.getElementById('typeTitle'+i).value += keyName;
    ...
    $return_var .= "<br /><p style='text-align: center'><input class='preview_button' type='button' value='see the results of this portion of the survey' onClick='previewResults(".count($my_types_arr).")'/></p>
